...
aliddns_token=`nvram get aliddns_token`
aliddns_Email=`nvram get aliddns_Email`
aliddns_Key=`nvram get aliddns_Key`
aliddns_domian=`nvram get aliddns_domian`
aliddns_host=`nvram get aliddns_host`
aliddns_domian2=`nvram get aliddns_domian2`
aliddns_host2=`nvram get aliddns_host2`
aliddns_domian6=`nvram get aliddns_domian6`
aliddns_host6=`nvram get aliddns_host6`
aliddns_interval=`nvram get aliddns_interval`

if [ ! -z "$aliddns_token" ] ; then
account_key_1="Authorization: Bearer $aliddns_token"
account_key_2=""
account_key_a1=" -H "
account_key_a2=""
fi
if [ -z "$aliddns_token" ] && [ ! -z "$aliddns_Email" ] && [ ! -z "$aliddns_Key" ] ; then
account_key_1="X-Auth-Email: $aliddns_Email"
account_key_2="X-Auth-Key: $aliddns_Key"
account_key_a1=" -H "
account_key_a2=" -H "
fi


if [ "$aliddns_domian"x != "x" ] && [ "$aliddns_host"x = "x" ] ; then
	aliddns_host="www"
	nvram set aliddns_host="www"
fi
if [ "$aliddns_domian2"x != "x" ] && [ "$aliddns_host2"x = "x" ] ; then
	aliddns_host2="www"
	nvram set aliddns_host2="www"
fi
if [ "$aliddns_domian6"x != "x" ] && [ "$aliddns_host6"x = "x" ] ; then
	aliddns_host6="www"
	nvram set aliddns_host6="www"
fi
...
B_restart="$aliddns_enable$aliddns_token$aliddns_Email$aliddns_Key$aliddns_domian$aliddns_host$aliddns_domian2$aliddns_host2$aliddns_domian6$aliddns_host6$aliddns_interval$(cat /etc/storage/ddns_script.sh | grep -v '^#' | grep -v "^$")"
...
[ -z "\`pidof Sh44_aliddns.sh\`" ] && nvram set aliddns_status=00 && logger -t "【aliddns】" "重新启动" && eval "$scriptfilepath &" && sed -Ei '/【aliddns】|^$/d' /tmp/script/_opt_script_check # 【aliddns】
OSC
while true; do
sleep 43
